    ...
    <Shell>
      <div className="mx-auto max-w-lg">
        ...
        <form onSubmit={submit}>
          ...
          <Button className="mt-6 w-full" disabled={createProject.isPending || isReadingFile}>
            {createProject.isPending ? "Creating project..." : isReadingFile ? "Reading file..." : "Create project ->"}
          </Button>
        </form>
      </div>
      {(createProject.isPending || isReadingFile) && (
        <StatusToast
          title={createProject.isPending ? "Creating project" : "Reading text file"}
          description={createProject.isPending ? "Saving the book text and preparing the pipeline." : "Loading the selected .txt file."}
        />
      )}
    </Shell>
  );
